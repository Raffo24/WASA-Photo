...
func (rt *_router) youAreNotAuthorized(r *http.Request, w http.ResponseWriter) (bool, int) {
	...
	if !bool {
		w.WriteHeader(400)
		w.Write([]byte("utente non presente"))
		return true, 0
	}
	return false, myID
}
...
func (rt *_router) changeMyNameHandler(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if myID != id {
		w.WriteHeader(401)
		w.Write([]byte("Non puoi cambiare il nome di un altro utente"))
		return
	}
	if err != nil {
		w.WriteHeader(400)
...
func (rt *_router) uploadPhotoHandler(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if err != nil {
		w.WriteHeader(400)
		w.Write([]byte("error reading photo"))
		return
	}
	...
	if err != nil {
		w.WriteHeader(400)
		w.Write([]byte("error create file"))
		return
	}
	defer f.Close()
...
func (rt *_router) likePhotoHandler(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if myID != userID {
		w.WriteHeader(401)
		w.Write([]byte("Non puoi mettere like da un altro utente"))
		return
	}
...
func (rt *_router) followUserHandler(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if myID != followerID {
		w.WriteHeader(401)
		w.Write([]byte("Non puoi seguire utilizzando un altro utente"))
		return
	}
	if rt.youAreBanned(myID, followingID, r, w) {
		w.WriteHeader(401)
		w.Write([]byte("Non puoi seguire un utente che ti ha bannato"))
		return
	}
...
func (rt *_router) banUserHandler(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if myID != bannerID {
		w.WriteHeader(401)
		w.Write([]byte("Non puoi bannare utilizzando un altro utente"))
		return
	}
...
func (rt *_router) deleteUserHandler(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if myID != userID {
		w.WriteHeader(401)
		w.Write([]byte("Non puoi cancellare un utente che non ti appartiene"))
		return
	}
...
func (rt *_router) deleteCommentHandler(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if comment.UserID != myID {
		w.WriteHeader(401)
		w.Write([]byte("Non puoi cancellare un commento che non ti appartiene"))
		return
	}
...
func (rt *_router) deletePhotoHandler(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if photo.UserID != myID {
		w.WriteHeader(401)
		w.Write([]byte("Non puoi cancellare una foto che non ti appartiene"))
		return
	}
...
func (rt *_router) unlikePhotoHandler(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if myID != userID {
		w.WriteHeader(401)
		w.Write([]byte("Non puoi levare un like che non hai messo tu"))
		return
	}
...
func (rt *_router) unfollowUserHandler(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if myID != followerID {
		w.WriteHeader(401)
		w.Write([]byte("Non puoi smettere di seguire da un account che non ti appartiene"))
		return
	}
	followingID, err := strconv.Atoi(ps.ByName("followId"))
	if err != nil {
...
func (rt *_router) unbanUserHandler(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if myID != bannerID {
		w.WriteHeader(401)
		w.Write([]byte("Non puoi sbannare da un account che non ti appartiene"))
		return
	}
	bannedID, err := strconv.Atoi(ps.ByName("banId"))
	if err != nil {
